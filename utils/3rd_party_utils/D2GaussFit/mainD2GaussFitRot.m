...
xs=x0; %x = [2,2.2,7,3.4,4.5,+0.02*2*pi]; %centroid parameters
noise = 0; %noise = 10; % noise in % of centroid peak value (x(1))
...
xin = x;
noise = noise/100 * x(1);
% [X,Y] = meshgrid(x,y); %[X,Y] = meshgrid(-MdataSize/2:MdataSize/2);
% [X,Y] = meshgrid(-MdataSize/2:MdataSize/2);
[X,Y] = meshgrid([1,0],[0,1]);
...
Z = ones(5,5); %Z = zdata; %Z = D2GaussFunctionRot(x,xdata);
