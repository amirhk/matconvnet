...
% OTHER METRIC THAT WORKS...
% X_pca = U_pca * U_pca' * X;
% X_rpca = U_rpca * U_rpca' * X;
% X_rp = U_rp * U_rp' * X;
% one_dim_all_delta_pca_original(end+1) = norm(X - X_pca, 'fro') ^ 2;
% one_dim_all_delta_rpca_original(end+1) = norm(X - X_rpca, 'fro') ^ 2;
% one_dim_all_delta_rp_original(end+1) = norm(X - X_rp, 'fro') ^ 2;
