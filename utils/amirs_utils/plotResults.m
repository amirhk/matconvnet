...
fc_plus_5_2x2D_3xcompRand = load(fullfile(dataDir, subDataDir, 'cifar-alex-net-4-Nov-2016-17-29-44-GPU1', epochFile)); fprintf('\t%d', i); i = i + 1;
fc_plus_4_2x2D_3xcompRand = load(fullfile(dataDir, subDataDir, 'cifar-alex-net-4-Nov-2016-19-38-40-GPU1', epochFile)); fprintf('\t%d', i); i = i + 1;
fc_plus_3_2x2D_3xcompRand = load(fullfile(dataDir, subDataDir, 'cifar-alex-net-4-Nov-2016-21-40-27-GPU1', epochFile)); fprintf('\t%d', i); i = i + 1;
fc_plus_2_2x2D_3xcompRand = load(fullfile(dataDir, subDataDir, 'cifar-alex-net-4-Nov-2016-23-06-48-GPU1', epochFile)); fprintf('\t%d', i); i = i + 1;
fc_plus_1_2x2D_3xcompRand = load(fullfile(dataDir, subDataDir, 'cifar-alex-net-5-Nov-2016-00-21-57-GPU1', epochFile)); fprintf('\t%d', i); i = i + 1;
fc_only_2x2D_3xcompRand = load(fullfile(dataDir, subDataDir, 'cifar-alex-net-5-Nov-2016-01-28-38-GPU1', epochFile)); fprintf('\t%d', i); i = i + 1;

subDataDir = '2016-11-04-06; AlexNet; FC+{0-5}; 2x2D-mult + 3x1D';
fc_plus_5_2x2D_3x1D = load(fullfile(dataDir, subDataDir, 'cifar-alexnet-6-Nov-2016-22-02-25-GPU1', epochFile)); fprintf('\t%d', i); i = i + 1;
fc_plus_4_2x2D_3x1D = load(fullfile(dataDir, subDataDir, 'cifar-alexnet-7-Nov-2016-00-08-06-GPU1', epochFile)); fprintf('\t%d', i); i = i + 1;
fc_plus_3_2x2D_3x1D = load(fullfile(dataDir, subDataDir, 'cifar-alexnet-7-Nov-2016-02-05-24-GPU1', epochFile)); fprintf('\t%d', i); i = i + 1;
fc_plus_2_2x2D_3x1D = load(fullfile(dataDir, subDataDir, 'cifar-alexnet-7-Nov-2016-03-29-01-GPU1', epochFile)); fprintf('\t%d', i); i = i + 1;
fc_plus_1_2x2D_3x1D = load(fullfile(dataDir, subDataDir, 'cifar-alexnet-7-Nov-2016-04-41-09-GPU1', epochFile)); fprintf('\t%d', i); i = i + 1;
fc_only_2x2D_3x1D = load(fullfile(dataDir, subDataDir, 'cifar-alexnet-7-Nov-2016-05-45-33-GPU1', epochFile)); fprintf('\t%d', i); i = i + 1;
...
fc_only_5x1D = load(fullfile(dataDir, subDataDir, 'cifar-alexnet-7-Nov-2016-04-14-34-GPU2', epochFile)); fprintf('\t%d', i); i = i + 1;

fprintf('\nDone!');

h = figure;
plot( ...
  1:1:epochNum, [fc_plus_5_2x2D_3xcompRand.info.train.error(1,:)], 'y', ...
  1:1:epochNum, [fc_plus_4_2x2D_3xcompRand.info.train.error(1,:)], 'm', ...
  1:1:epochNum, [fc_plus_3_2x2D_3xcompRand.info.train.error(1,:)], 'c', ...
  1:1:epochNum, [fc_plus_2_2x2D_3xcompRand.info.train.error(1,:)], 'r', ...
  1:1:epochNum, [fc_plus_1_2x2D_3xcompRand.info.train.error(1,:)], 'g', ...
  1:1:epochNum, [fc_only_2x2D_3xcompRand.info.train.error(1,:)], 'b', ...
  1:1:epochNum, [fc_plus_5_2x2D_3x1D.info.train.error(1,:)], 'y--', ...
  1:1:epochNum, [fc_plus_4_2x2D_3x1D.info.train.error(1,:)], 'm--', ...
  1:1:epochNum, [fc_plus_3_2x2D_3x1D.info.train.error(1,:)], 'c--', ...
  1:1:epochNum, [fc_plus_2_2x2D_3x1D.info.train.error(1,:)], 'r--', ...
  1:1:epochNum, [fc_plus_1_2x2D_3x1D.info.train.error(1,:)], 'g--', ...
  1:1:epochNum, [fc_only_2x2D_3x1D.info.train.error(1,:)], 'b--', ...
  'LineWidth', 2);
grid on
title('Effects of Varying BackProp Depth on Training Accuracy');
legend(...
  'FC + 5 (FULL) (2x2D + 3xcompRand)', ...
  'FC + 4 (2x2D + 3xcompRand)', ...
  'FC + 3 (2x2D + 3xcompRand)', ...
  'FC + 2 (2x2D + 3xcompRand)', ...
  'FC + 1 (2x2D + 3xcompRand)', ...
  'FC ONLY (2x2D + 3xcompRand)', ...
  'FC + 5 (FULL) (2x2D + 3x1D)', ...
  'FC + 4 (2x2D + 3x1D)', ...
  'FC + 3 (2x2D + 3x1D)', ...
  'FC + 2 (2x2D + 3x1D)', ...
...
saveas(h, sprintf('Training Comparison - %d - 2x2D_3xcompRand vs 2x2D_3x1D.png', epochNum));

h = figure;
plot( ...
  1:1:epochNum, [fc_plus_5_2x2D_3xcompRand.info.val.error(1,:)], 'y', ...
  1:1:epochNum, [fc_plus_4_2x2D_3xcompRand.info.val.error(1,:)], 'm', ...
  1:1:epochNum, [fc_plus_3_2x2D_3xcompRand.info.val.error(1,:)], 'c', ...
  1:1:epochNum, [fc_plus_2_2x2D_3xcompRand.info.val.error(1,:)], 'r', ...
  1:1:epochNum, [fc_plus_1_2x2D_3xcompRand.info.val.error(1,:)], 'g', ...
  1:1:epochNum, [fc_only_2x2D_3xcompRand.info.val.error(1,:)], 'b', ...
  1:1:epochNum, [fc_plus_5_2x2D_3x1D.info.val.error(1,:)], 'y--', ...
  1:1:epochNum, [fc_plus_4_2x2D_3x1D.info.val.error(1,:)], 'm--', ...
  1:1:epochNum, [fc_plus_3_2x2D_3x1D.info.val.error(1,:)], 'c--', ...
  1:1:epochNum, [fc_plus_2_2x2D_3x1D.info.val.error(1,:)], 'r--', ...
  1:1:epochNum, [fc_plus_1_2x2D_3x1D.info.val.error(1,:)], 'g--', ...
  1:1:epochNum, [fc_only_2x2D_3x1D.info.val.error(1,:)], 'b--', ...
  'LineWidth', 2);
grid on
...
saveas(h, sprintf('Validation Comparison - %d - 2x2D_3xcompRand vs 2x2D_3x1D.png', epochNum));



h = figure;
plot( ...
  1:1:epochNum, [fc_plus_5_5x1D.info.train.error(1,:)], 'y-.', ...
  1:1:epochNum, [fc_plus_4_5x1D.info.train.error(1,:)], 'm-.', ...
  1:1:epochNum, [fc_plus_3_5x1D.info.train.error(1,:)], 'c-.', ...
  1:1:epochNum, [fc_plus_2_5x1D.info.train.error(1,:)], 'r-.', ...
  1:1:epochNum, [fc_plus_1_5x1D.info.train.error(1,:)], 'g-.', ...
  1:1:epochNum, [fc_only_5x1D.info.train.error(1,:)], 'b-.', ...
  1:1:epochNum, [fc_plus_5_2x2D_3x1D.info.train.error(1,:)], 'y--', ...
  1:1:epochNum, [fc_plus_4_2x2D_3x1D.info.train.error(1,:)], 'm--', ...
  1:1:epochNum, [fc_plus_3_2x2D_3x1D.info.train.error(1,:)], 'c--', ...
  1:1:epochNum, [fc_plus_2_2x2D_3x1D.info.train.error(1,:)], 'r--', ...
...
xlabel('epoch')
ylabel('Training Error');
saveas(h, sprintf('Training Comparison - %d - 5x1D vs 2x2D_3x1D.png', epochNum));

h = figure;
plot( ...
  1:1:epochNum, [fc_plus_5_5x1D.info.val.error(1,:)], 'y-.', ...
  1:1:epochNum, [fc_plus_4_5x1D.info.val.error(1,:)], 'm-.', ...
  1:1:epochNum, [fc_plus_3_5x1D.info.val.error(1,:)], 'c-.', ...
  1:1:epochNum, [fc_plus_2_5x1D.info.val.error(1,:)], 'r-.', ...
  1:1:epochNum, [fc_plus_1_5x1D.info.val.error(1,:)], 'g-.', ...
  1:1:epochNum, [fc_only_5x1D.info.val.error(1,:)], 'b-.', ...
  1:1:epochNum, [fc_plus_5_2x2D_3x1D.info.val.error(1,:)], 'y--', ...
  1:1:epochNum, [fc_plus_4_2x2D_3x1D.info.val.error(1,:)], 'm--', ...
  1:1:epochNum, [fc_plus_3_2x2D_3x1D.info.val.error(1,:)], 'c--', ...
  1:1:epochNum, [fc_plus_2_2x2D_3x1D.info.val.error(1,:)], 'r--', ...
...
xlabel('epoch')
ylabel('Validation Error');
saveas(h, sprintf('Validation Comparison - %d - 5x1D vs 2x2D_3x1D.png', epochNum));
...
title('Effects of Varying BackProp Depth on Training Accuracy');
legend(...
  'training FC ONLY (5x1D)', ...
  'training FC ONLY (2x2D + 3xcompRand)', ...
  'training FC ONLY (2x2D + 3x1D)');
xlabel('epoch')
ylabel('Training Error');
saveas(h, sprintf('Training Comparison - %d - 5x1D vs 2x2D_3x1D vs 2x2D_3xcompRand.png', epochNum));
...
xlabel('epoch')
ylabel('Validation Error');
saveas(h, sprintf('Validation Comparison - %d - 5x1D vs 2x2D_3x1D vs 2x2D_3xcompRand.png', epochNum));
