% -------------------------------------------------------------------------
function imdb = constructProstateImdb2(opts)
% -------------------------------------------------------------------------
  fprintf('[INFO] Loading Prostate imdb...\n');
  imdb = load(opts.imdbBalancedPath);
  fprintf('[INFO] Changing leave-1-out-sample to sample #%d...\n', opts.patientNumber);
  i = opts.patientNumber;
  imdb.images.set(i) = 3;
  % becuase only setting 1 sample, cnn_train will fail. needs at least values from tumor and not tumor
  if imdb.images.labels(i) == 2 % tumor
    % find all non-tumors
    tmp = find(imdb.images.labels == 1);
    % choose one at random
    ix = randperm(length(tmp));
    index = ix(1);
    % make that index also part of test set
    imdb.images.set(index) = 3;
  else
    % find all tumors
    tmp = find(imdb.images.labels == 2);
    % choose one at random
    ix = randperm(length(tmp));
    index = ix(1);
    % make that index also part of test set
...
%   set = [1*ones(1,length(labels_train)) 1*ones(1,length(labels_test))];
